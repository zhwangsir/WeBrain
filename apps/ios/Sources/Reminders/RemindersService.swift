...
final class RemindersService: RemindersServicing {
    func list(params: WineryClawRemindersListParams) async throws -> WineryClawRemindersListPayload {
        ...
        let payload: [WineryClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return WineryClawReminderPayload(
        ...
        return WineryClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: WineryClawRemindersAddParams) async throws -> WineryClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = WineryClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return WineryClawRemindersAddPayload(reminder: payload)
    }
