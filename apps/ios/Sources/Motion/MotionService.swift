...
final class MotionService: MotionServicing {
    func activities(params: WineryClawMotionActivityParams) async throws -> WineryClawMotionActivityPayload {
        ...
        let mapped: [WineryClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return WineryClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: WineryClawPedometerParams) async throws -> WineryClawPedometerPayload {
        ...
        let payload: WineryClawPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = WineryClawPedometerPayload(
