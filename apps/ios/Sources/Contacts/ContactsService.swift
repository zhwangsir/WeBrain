...
final class ContactsService: ContactsServicing {
    ...
    func search(params: WineryClawContactsSearchParams) async throws -> WineryClawContactsSearchPayload {
        ...
        return WineryClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: WineryClawContactsAddParams) async throws -> WineryClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return WineryClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return WineryClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> WineryClawContactPayload {
        WineryClawContactPayload(
