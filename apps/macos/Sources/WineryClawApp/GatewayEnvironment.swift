...
enum GatewayEnvironment {
    ...
    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["WINERYCLAW_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = WineryClawConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["WINERYCLAW_GATEWAY_BIND"] {
