...
enum WineryClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = WineryClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = WineryClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        WineryClawLogging.bootstrapIfNeeded()
        let label = WineryClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct WineryClawOSLogHandler: AppLogLevelBackedHandler {
...
struct WineryClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = WineryClawLogging.parseLabel(self.label)
