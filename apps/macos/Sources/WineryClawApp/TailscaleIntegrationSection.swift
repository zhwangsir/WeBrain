...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.wineryclaw/wineryclaw.json. Prefer WINERYCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.wineryclaw/wineryclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.wineryclaw/wineryclaw.json. Restart the gateway to apply."
        }
