...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> WineryClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: WineryClawSessionPreviewEntry,
    ...
    private static func cache(payload: WineryClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: WineryClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [WineryClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [WineryClawKit.AnyCodable]) -> [WineryClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(WineryClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: WineryClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: WineryClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: WineryClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: WineryClawChatMessage) -> String? {
