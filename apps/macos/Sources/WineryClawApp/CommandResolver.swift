...
enum CommandResolver {
    ...
    static func projectWineryClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func hasAnyWineryClawInvoker(searchPaths: [String]? = nil) -> Bool {
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let openclawPath = self.projectWineryClawExecutable(projectRoot: root) {
    ...
    {
        let root = configRoot ?? WineryClawConfigFile.loadDict()
