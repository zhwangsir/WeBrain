...
enum RemoteGatewayAuthIssue: Equatable {
    ...
    var body: String {
        switch self {
        case .tokenRequired:
            "Paste the token configured on the gateway host. On the gateway host, run `openclaw config get gateway.auth.token`. If the gateway uses an environment variable instead, use `WINERYCLAW_GATEWAY_TOKEN`."
        case .tokenMismatch:
            "Check `gateway.auth.token` or `WINERYCLAW_GATEWAY_TOKEN` on the gateway host and try again."
        case .gatewayTokenNotConfigured:
            "This gateway is set to token auth, but no `gateway.auth.token` is configured on the gateway host. If the gateway uses an environment variable instead, set `WINERYCLAW_GATEWAY_TOKEN` before starting the gateway."
        case .setupCodeExpired:
            "Scan or paste a fresh setup code from an already-paired WineryClaw client, then try again."
        case .passwordRequired:
            "This onboarding flow does not support password auth yet. Reconfigure the gateway to use token auth, then retry."
        case .pairingRequired:
            "Approve this device from an already-paired WineryClaw client. In your WineryClaw chat, run `/pair approve`, then click **Check connection** again."
        }
    }
    ...
    var footnote: String? {
        ...
        case .setupCodeExpired:
            nil
        case .pairingRequired:
            "If you do not have another paired WineryClaw client yet, approve the pending request on the gateway host with `openclaw devices approve`."
    ...
    var statusMessage: String {
        ...
        case .tokenMismatch:
            "Gateway token mismatch. Check gateway.auth.token or WINERYCLAW_GATEWAY_TOKEN on the gateway host."
        case .gatewayTokenNotConfigured:
        ...
        case .pairingRequired:
            "Pairing required. In an already-paired WineryClaw client, run /pair approve, then check the connection again."
        }
    }
}
