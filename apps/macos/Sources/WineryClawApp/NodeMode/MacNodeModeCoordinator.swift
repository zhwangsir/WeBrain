...
final class MacNodeModeCoordinator {
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = WineryClawConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: WineryClawNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [WineryClawCapability.canvas.rawValue, WineryClawCapability.screen.rawValue]
        if WineryClawConfigFile.browserControlEnabled() {
            caps.append(WineryClawCapability.browser.rawValue)
        }
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(WineryClawCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if WineryClawLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(WineryClawCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            WineryClawSystemCommand.which.rawValue,
            WineryClawSystemCommand.run.rawValue,
            WineryClawSystemCommand.execApprovalsGet.rawValue,
            WineryClawSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(WineryClawCapability.browser.rawValue) {
            commands.append(WineryClawBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(WineryClawCapability.camera.rawValue) {
        ...
        if capsSet.contains(WineryClawCapability.location.rawValue) {
