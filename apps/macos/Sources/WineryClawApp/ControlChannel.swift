...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: WineryClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: WineryClawKit.AnyCodable]()) {
                $0[$1.key] = WineryClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: WineryClawProtocol.AnyCodable?) -> [String: WineryClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: WineryClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: WineryClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: WineryClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
