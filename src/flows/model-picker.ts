...
export type PromptDefaultModelResult = { model?: string; config?: WineryClawConfig };
...
function resolveConfiguredModelRaw(cfg: WineryClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: WineryClawConfig): string[] {
...
export function applyModelAllowlist(cfg: WineryClawConfig, models: string[]): WineryClawConfig {
