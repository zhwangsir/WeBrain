...
import type { WineryClawConfig, ConfigFileSnapshot } from "../config/config.js";
...
function readChannelCommandSetting(
  ...
  const channelCfg = cfg.channels?.[channelId as keyof NonNullable<WineryClawConfig["channels"]>];
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your WineryClaw extensions state directory.",
      });
