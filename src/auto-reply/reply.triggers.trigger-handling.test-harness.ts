...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    openclawHome: process.env.WINERYCLAW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("WINERYCLAW_HOME", snapshot.wineryclawHome);
  restoreKey("WINERYCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.WINERYCLAW_STATE_DIR = join(home, ".wineryclaw");
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".wineryclaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): WineryClawConfig {
