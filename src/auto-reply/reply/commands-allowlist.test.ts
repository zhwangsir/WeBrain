...
async function withTempConfigPath<T>(
  ...
  const previous = process.env.WINERYCLAW_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.WINERYCLAW_CONFIG_PATH = previous;
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<WineryClawConfig>(configPath);
