...
const { buildStatusReplyMock, createWineryClawToolsMock, getChannelPluginMock, handleCommandsMock } =
  vi.hoisted(() => ({
    buildStatusReplyMock: vi.fn(),
    createWineryClawToolsMock: vi.fn(),
...
vi.mock("../../agents/openclaw-tools.runtime.js", () => ({
  createWineryClawTools: (...args: unknown[]) => createWineryClawToolsMock(...args),
}));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createWineryClawToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
