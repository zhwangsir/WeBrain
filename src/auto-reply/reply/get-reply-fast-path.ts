...
type ReplyConfigWithMarker = WineryClawConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.WINERYCLAW_ALLOW_SLOW_REPLY_TESTS === "1" || env.WINERYCLAW_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends WineryClawConfig>(
  config: T,
...
export function withFastReplyConfig<T extends WineryClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends WineryClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

export function isCompleteReplyConfig(config: unknown): config is WineryClawConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set WINERYCLAW_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  if (params.isFastTestEnv && isCompleteReplyConfig(configOverride)) {
    return configOverride;
  }
  return applyMergePatch(params.loadConfig(), configOverride) as WineryClawConfig;
}
