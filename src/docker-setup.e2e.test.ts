...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    WINERYCLAW_GATEWAY_TOKEN: "test-token",
    WINERYCLAW_CONFIG_DIR: join(sandbox.rootDir, "config"),
    WINERYCLAW_WORKSPACE_DIR: join(sandbox.rootDir, "openclaw"),
  };
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    ...
    WINERYCLAW_CONFIG_DIR: configDir,
    WINERYCLAW_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      WINERYCLAW_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      WINERYCLAW_EXTRA_MOUNTS: undefined,
      WINERYCLAW_HOME_VOLUME: "openclaw-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("WINERYCLAW_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("WINERYCLAW_EXTRA_MOUNTS=");
    expect(envFile).toContain("WINERYCLAW_HOME_VOLUME=openclaw-home"); // pragma: allowlist secret
    ...
    expect(log).toContain("--build-arg WINERYCLAW_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  ...
  it("writes WINERYCLAW_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      WINERYCLAW_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("WINERYCLAW_TZ=Asia/Shanghai");
  });
  ...
  it("reuses existing config token when WINERYCLAW_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      ...
      async (configDir) => {
        await writeFile(
          join(configDir, "wineryclaw.json"),
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("WINERYCLAW_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when WINERYCLAW_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "WINERYCLAW_GATEWAY_TOKEN=dotenv-token-123\nWINERYCLAW_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("WINERYCLAW_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "WINERYCLAW_GATEWAY_TOKEN=first-token",
        "WINERYCLAW_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("WINERYCLAW_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("WINERYCLAW_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats WINERYCLAW_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      WINERYCLAW_SANDBOX: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("WINERYCLAW_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg WINERYCLAW_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg WINERYCLAW_INSTALL_DOCKER_CLI=1");
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        WINERYCLAW_SANDBOX: "1",
        WINERYCLAW_DOCKER_SOCKET: socketPath,
  ...
  it("rejects injected multiline WINERYCLAW_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      WINERYCLAW_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("WINERYCLAW_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid WINERYCLAW_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      WINERYCLAW_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid WINERYCLAW_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      WINERYCLAW_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("WINERYCLAW_HOME_VOLUME must match");
  });

  it("rejects WINERYCLAW_TZ values that are not present in zoneinfo", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      WINERYCLAW_TZ: "Nope/Bad",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("WINERYCLAW_TZ must match a timezone in /usr/share/zoneinfo");
  });
  ...
  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/WINERYCLAW_GATEWAY_TOKEN: \$\{WINERYCLAW_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{WINERYCLAW_TZ:-UTC\}/g)).toHaveLength(2);
  });
});
