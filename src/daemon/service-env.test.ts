...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.WINERYCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.WINERYCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.WINERYCLAW_SERVICE_MARKER).toBe("openclaw");
    expect(env.WINERYCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.WINERYCLAW_SERVICE_VERSION).toBe("string");
    expect(env.WINERYCLAW_SYSTEMD_UNIT).toBe("openclaw-gateway.service");
    expect(env.WINERYCLAW_WINDOWS_TASK_NAME).toBe("WineryClaw Gateway");
    if (process.platform === "darwin") {
      expect(env.WINERYCLAW_LAUNCHD_LABEL).toBe("ai.openclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", WINERYCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.WINERYCLAW_SYSTEMD_UNIT).toBe("openclaw-gateway-work.service");
    expect(env.WINERYCLAW_WINDOWS_TASK_NAME).toBe("WineryClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.WINERYCLAW_LAUNCHD_LABEL).toBe("ai.openclaw.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through WINERYCLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", WINERYCLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.WINERYCLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits WINERYCLAW_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".wineryclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", WINERYCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", WINERYCLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".wineryclaw"));
  });

  it("uses WINERYCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", WINERYCLAW_STATE_DIR: "/var/lib/openclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openclaw"));
  });

  it("expands ~ in WINERYCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", WINERYCLAW_STATE_DIR: "~/openclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { WINERYCLAW_STATE_DIR: "C:\\State\\openclaw" };
