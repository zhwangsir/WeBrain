...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when WINERYCLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".wineryclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when WINERYCLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", WINERYCLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".openclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers WINERYCLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        WINERYCLAW_PROFILE: "rescue",
        WINERYCLAW_STATE_DIR: "C:\\State\\openclaw",
      },
      expected: path.join("C:\\State\\openclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", WINERYCLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".wineryclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem WineryClaw Gateway",
          "cd /d C:\\Projects\\openclaw",
          "set NODE_ENV=production",
          "set WINERYCLAW_PORT=18789",
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\WineryClaw Share\\node.exe" "\\\\fileserver\\WineryClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\WineryClaw Share\\node.exe",
            "\\\\fileserver\\WineryClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from WINERYCLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ WINERYCLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
