...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: WineryClawConfig, policy: ChannelAccessPolicy) => WineryClawConfig;
  resolveAllowlist: (params: { cfg: WineryClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: WineryClawConfig; resolved: TResolved }) => WineryClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: WineryClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: WineryClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: WineryClawConfig, policy: ChannelAccessPolicy): WineryClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: WineryClawConfig, policy: ChannelAccessPolicy): WineryClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: WineryClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: WineryClawConfig; resolved: string[] }) => {
      calls.push("apply");
