...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: WineryClawConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: WineryClawConfig }>(
  collector: WarningCollector<{ cfg: WineryClawConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: WineryClawConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: WineryClawConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: WineryClawConfig },
>(
  projectCfg: (cfg: WineryClawConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: WineryClawConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: WineryClawConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: WineryClawConfig },
>(params: {
  providerConfigPresent: (cfg: WineryClawConfig) => boolean;
