...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): WineryClawConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as WineryClawConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: WineryClawConfig) => string[];
  input: WineryClawConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: WineryClawConfig, expected: string) {
