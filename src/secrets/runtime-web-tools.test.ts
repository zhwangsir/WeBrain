...
function setConfiguredFetchProviderKey(configTarget: WineryClawConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: WineryClawConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: WineryClawConfig, provider: ProviderUnderTest): unknown {
