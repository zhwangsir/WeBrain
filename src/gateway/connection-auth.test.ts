...
function cfg(input: Partial<WineryClawConfig>): WineryClawConfig {
  return input as WineryClawConfig;
}
...
const DEFAULT_ENV = {
  ...
  WINERYCLAW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when WINERYCLAW env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when WINERYCLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when WINERYCLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
