...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([WINERYCLAW_MODEL_ID, WINERYCLAW_DEFAULT_MODEL_ID]);
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== WINERYCLAW_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
