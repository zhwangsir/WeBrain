...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.WINERYCLAW_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: WineryClawConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: WineryClawConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: WineryClawConfig, requestedAgentId: string) => {
