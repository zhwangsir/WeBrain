...
import type { ModelsConfig, WineryClawConfig, ModelProviderConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.WINERYCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.WINERYCLAW_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.WINERYCLAW_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.WINERYCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.WINERYCLAW_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.WINERYCLAW_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["WINERYCLAW_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.WINERYCLAW_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.WINERYCLAW_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.WINERYCLAW_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.WINERYCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.WINERYCLAW_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.WINERYCLAW_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.WINERYCLAW_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.WINERYCLAW_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("WINERYCLAW_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("WINERYCLAW_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("WINERYCLAW_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.WINERYCLAW_LIVE_GATEWAY_MODELS;
    delete process.env.WINERYCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.WINERYCLAW_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.WINERYCLAW_LIVE_GATEWAY_MODELS = "openai/gpt-5.4,anthropic/claude-opus-4-6";
    delete process.env.WINERYCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.WINERYCLAW_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.WINERYCLAW_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function sanitizeAuthConfig(params: {
  cfg: WineryClawConfig;
  agentDir: string;
}): Promise<WineryClawConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<WineryClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    ...
    token: process.env.WINERYCLAW_GATEWAY_TOKEN,
    ...
    disableBonjour: process.env.WINERYCLAW_DISABLE_BONJOUR,
    logLevel: process.env.WINERYCLAW_LOG_LEVEL,
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.WINERYCLAW_DISABLE_BONJOUR = "1";
    process.env.WINERYCLAW_LOG_LEVEL = "silent";
  }
  ...
  const hostAgentDir = resolveWineryClawAgentDir();
  ...
  const sanitizedCfg: WineryClawConfig = {
    ...params.cfg,
  ...
  const tempConfigPath = path.join(tempDir, "wineryclaw.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.WINERYCLAW_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "WineryClaw live tool probe (local, safe): " +
    ...
    process.env.WINERYCLAW_CONFIG_PATH = previous.configPath;
    process.env.WINERYCLAW_GATEWAY_TOKEN = previous.token;
    process.env.WINERYCLAW_SKIP_CHANNELS = previous.skipChannels;
    process.env.WINERYCLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.WINERYCLAW_SKIP_CRON = previous.skipCron;
    process.env.WINERYCLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.WINERYCLAW_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.WINERYCLAW_LOG_LEVEL = previous.logLevel;
    process.env.WINERYCLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.WINERYCLAW_STATE_DIR = previous.stateDir;
  }
}
  ...
  it(
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        await ensureWineryClawModelsJson(cfg);
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via WINERYCLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
