...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses WINERYCLAW_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/openclaw-home"), ".wineryclaw", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("archiveSessionTranscripts", () => {
  ...
  beforeAll(() => {
    vi.stubEnv("WINERYCLAW_HOME", tmpDir);
  });
