...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): WineryClawConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: WineryClawConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        WINERYCLAW_STATE_DIR: envStateDir,
      };
      const cfg: WineryClawConfig = {};
