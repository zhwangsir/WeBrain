...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".wineryclaw", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: WineryClawConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [
    ["WINERYCLAW_GATEWAY_TOKEN", await getenv("WINERYCLAW_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["WINERYCLAW_GATEWAY_PASSWORD", await getenv("WINERYCLAW_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "WINERYCLAW_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.WINERYCLAW_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- WINERYCLAW_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
