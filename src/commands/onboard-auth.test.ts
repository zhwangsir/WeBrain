...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under WINERYCLAW_AGENT_DIR when set", async () => {
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    ...
    process.env.WINERYCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("syncs siblings from explicit agentDir outside WINERYCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-oauth-external-"));
    process.env.WINERYCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* WINERYCLAW_STATE_DIR
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to WINERYCLAW_AGENT_DIR when set", async () => {
