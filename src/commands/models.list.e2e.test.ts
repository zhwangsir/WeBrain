...
const ensureWineryClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveWineryClawAgentDir = vi.fn().mockReturnValue("/tmp/openclaw-agent");
...
beforeEach(() => {
  ...
  ensureWineryClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureWineryClawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the source snapshot config when provided", async () => {
    ...
    expect(ensureWineryClawModelsJson).toHaveBeenCalled();
    expect(ensureWineryClawModelsJson.mock.calls[0]?.[0]).toEqual(sourceConfig);
  });
