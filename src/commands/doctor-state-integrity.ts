...
function listOrphanAgentDirs(cfg: WineryClawConfig, stateDir: string): OrphanAgentDir[] {
...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".wineryclaw"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for WINERYCLAW_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // WINERYCLAW_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: WineryClawConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.WINERYCLAW_OAUTH_DIR?.trim()) {
    return true;
  }
...
function shouldSuppressOrphanTranscriptWarning(cfg: WineryClawConfig, agentId: string): boolean {
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".wineryclaw");
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        `  Set locally: WINERYCLAW_STATE_DIR=~/.openclaw ${formatCliCommand("openclaw doctor")}`,
      ].join("\n"),
    );
  }
