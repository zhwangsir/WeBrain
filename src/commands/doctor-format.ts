...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.cachedLabel && platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(env.WINERYCLAW_PROFILE);
    hints.push(
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        platform,
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.WINERYCLAW_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.WINERYCLAW_PROFILE),
      }),
    );
  }
  return hints;
