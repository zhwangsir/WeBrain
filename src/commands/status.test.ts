...
beforeAll(() => {
  envSnapshot = captureEnv(["WINERYCLAW_PROFILE"]);
  process.env.WINERYCLAW_PROFILE = "isolated";
});
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.WINERYCLAW_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/openclaw-root.js", () => ({
  resolveWineryClawPackageRoot: vi.fn().mockResolvedValue("/tmp/openclaw"),
...
vi.mock("./status.daemon.js", () => ({
  getDaemonStatusSummary: vi.fn(async () => {
    ...
    return {
      ...
      loaded,
      managedByWineryClaw: Boolean(command),
...
describe("statusCommand", () => {
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("WINERYCLAW_GATEWAY_TOKEN", "abcd1234", async () => {
