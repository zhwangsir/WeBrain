...
export function hasExplicitMemorySearchConfig(cfg: WineryClawConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: WineryClawConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: WineryClawConfig, agentId: string) => { store: { path: string } } | null;
