...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { WINERYCLAW_PORT: "3000" },
  } = params;
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.wineryclaw/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ WINERYCLAW_PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.WINERYCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("CUSTOM_VAR,GOOGLE_API_KEY");
    // Service environment vars should take precedence
    expect(plan.environment.WINERYCLAW_PORT).toBe("3000");
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("merges .env file vars into the install plan", async () => {
    await writeStateDirDotEnv("BRAVE_API_KEY=BSA-from-env\nOPENROUTER_API_KEY=or-key\n", {
      stateDir: path.join(tmpDir, ".wineryclaw"),
    });
    mockNodeGatewayPlanFixture({ serviceEnvironment: { WINERYCLAW_PORT: "3000" } });
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        WINERYCLAW_SERVICE_MARKER: "openclaw",
      },
    });
    ...
    expect(plan.environment.WINERYCLAW_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        WINERYCLAW_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
    ...
    expect(plan.environment.BLOGWATCHER_HOME).toBe("/Users/test/.blogwatcher");
    expect(plan.environment.GOPATH).toBeUndefined();
    expect(plan.environment.WINERYCLAW_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
