...
import { WINERYCLAW_CLI_ENV_VALUE } from "../infra/openclaw-exec-env.js";
...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        WINERYCLAW_BASE_ENV: "base",
        WINERYCLAW_TO_REMOVE: undefined,
      },
      env: {
        WINERYCLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.WINERYCLAW_BASE_ENV).toBe("base");
    expect(resolved.WINERYCLAW_TEST_ENV).toBe("ok");
    expect(resolved.WINERYCLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.WINERYCLAW_CLI).toBe(WINERYCLAW_CLI_ENV_VALUE);
  });
