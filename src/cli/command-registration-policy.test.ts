...
describe("command-registration-policy", () => {
  ...
  it("matches lazy subcommand registration policy", () => {
    expect(shouldEagerRegisterSubcommands({ WINERYCLAW_DISABLE_LAZY_SUBCOMMANDS: "1" })).toBe(true);
    expect(shouldEagerRegisterSubcommands({ WINERYCLAW_DISABLE_LAZY_SUBCOMMANDS: "0" })).toBe(false);
    ...
    expect(
      shouldRegisterPrimarySubcommandOnly(["node", "openclaw", "acp"], {
        WINERYCLAW_DISABLE_LAZY_SUBCOMMANDS: "1",
      }),
    ).toBe(false);
  });
});
