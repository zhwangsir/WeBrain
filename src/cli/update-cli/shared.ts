...
const WINERYCLAW_REPO_URL = "https://github.com/openclaw/openclaw.git";
...
export function resolveGitInstallDir(): string {
  const override = process.env.WINERYCLAW_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveWineryClawPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", WINERYCLAW_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `WINERYCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set WINERYCLAW_GIT_DIR to an empty folder or an openclaw checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`WINERYCLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
