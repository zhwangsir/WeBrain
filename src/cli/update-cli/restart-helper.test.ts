...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("uses WINERYCLAW_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        WINERYCLAW_PROFILE: "default",
        WINERYCLAW_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("uses WINERYCLAW_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        WINERYCLAW_PROFILE: "default",
        WINERYCLAW_LAUNCHD_LABEL: "com.custom.openclaw",
      });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "WineryClaw Gateway"');
      expect(content).toContain('schtasks /Run /TN "WineryClaw Gateway"');
    ...
    it("uses WINERYCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      ...
      expect(content).toContain('schtasks /End /TN "WineryClaw Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "WineryClaw Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "WineryClaw Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("escapes single quotes in profile names for shell scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        WINERYCLAW_PROFILE: "it's-a-test",
      });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        WINERYCLAW_LAUNCHD_LABEL: "ai.openclaw.it's-a-test",
      });
    ...
    it("rejects unsafe batch profile names on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      const scriptPath = await prepareRestartScript({
        WINERYCLAW_PROFILE: "test&whoami",
      });
