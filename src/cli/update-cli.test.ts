...
import type { WineryClawConfig, ConfigFileSnapshot } from "../config/types.openclaw.js";
...
vi.mock("../infra/openclaw-root.js", () => ({
  resolveWineryClawPackageRoot: vi.fn(),
  resolveWineryClawPackageRootSync: vi.fn(() => process.cwd()),
}));
...
const { resolveWineryClawPackageRoot } = await import("../infra/openclaw-root.js");
...
describe("update-cli", () => {
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveWineryClawPackageRoot).mockResolvedValue(root);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveWineryClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          WINERYCLAW_UPDATE_POST_CORE: "1",
          WINERYCLAW_UPDATE_POST_CORE_CHANNEL: "dev",
        }),
      }),
    );
  ...
  it("post-core resume mode skips the core update and only runs post-update tasks", async () => {
    await withEnvAsync(
      {
        WINERYCLAW_UPDATE_POST_CORE: "1",
        WINERYCLAW_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("WineryClaw update status");
      },
    },
    {
      name: "json output",
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as WineryClawConfig,
        });
      },
  ...
  it.each([
    ...
    {
      name: "WINERYCLAW_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("openclaw-update"));
        await withEnvAsync(
          { WINERYCLAW_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/openclaw-next.tgz" },
          async () => {
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            WINERYCLAW_CONFIG_PATH: "./config/openclaw.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          cwd: root,
          env: expect.objectContaining({
            WINERYCLAW_STATE_DIR: path.resolve("./state"),
            WINERYCLAW_CONFIG_PATH: path.resolve("./config/openclaw.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            WINERYCLAW_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ WINERYCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.WINERYCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    ...
    await withEnvAsync({ WINERYCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/openclaw as the default dev checkout directory", async () => {
    ...
    await withEnvAsync({ WINERYCLAW_GIT_DIR: undefined }, async () => {
