...
describe("renderGatewayServiceStartHints", () => {
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        WINERYCLAW_CONTAINER: "openclaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("openclaw-demo-container");
    expect(
      resolveDaemonContainerContext({
        WINERYCLAW_CONTAINER_HINT: "openclaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("openclaw-demo-container");
  });

  it("prepends a single container restart hint when WINERYCLAW_CONTAINER is set", () => {
    expect(
      renderGatewayServiceStartHints({
        WINERYCLAW_CONTAINER: "openclaw-demo-container",
      } as NodeJS.ProcessEnv),
  ...
  it("prepends a single container restart hint when WINERYCLAW_CONTAINER_HINT is set", () => {
    expect(
      renderGatewayServiceStartHints({
        WINERYCLAW_CONTAINER_HINT: "openclaw-demo-container",
      } as NodeJS.ProcessEnv),
...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when WINERYCLAW_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
        ...
        { WINERYCLAW_CONTAINER: "openclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toEqual([]);
  });

  it("drops the generic container foreground hint when WINERYCLAW_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
        ...
        { WINERYCLAW_CONTAINER_HINT: "openclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toEqual([]);
  });
});
