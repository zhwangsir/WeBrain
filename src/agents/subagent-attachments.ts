...
function resolveAttachmentLimits(config: WineryClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".wineryclaw", "attachments");
  const relDir = path.posix.join(".wineryclaw", "attachments", attachmentId);
