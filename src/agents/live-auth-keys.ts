...
const GOOGLE_LIVE_SINGLE_KEY = "WINERYCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "WINERYCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "WINERYCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "WINERYCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `WINERYCLAW_LIVE_${base}_KEY`;
