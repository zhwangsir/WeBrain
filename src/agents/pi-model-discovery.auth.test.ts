...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.WINERYCLAW_AUTH_STORE_READONLY;
      ...
      try {
        ...
        if (previous === undefined) {
          delete process.env.WINERYCLAW_AUTH_STORE_READONLY;
        } else {
          process.env.WINERYCLAW_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });
  ...
  it("includes env-backed provider auth when no auth profile exists", async () => {
    ...
    const previousBundledPluginsDir = process.env.WINERYCLAW_BUNDLED_PLUGINS_DIR;
    const previousDisableBundledPlugins = process.env.WINERYCLAW_DISABLE_BUNDLED_PLUGINS;
    ...
    try {
      ...
      if (previousBundledPluginsDir === undefined) {
        ...
        process.env.WINERYCLAW_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
      }
      if (previousDisableBundledPlugins === undefined) {
        delete process.env.WINERYCLAW_DISABLE_BUNDLED_PLUGINS;
      } else {
        process.env.WINERYCLAW_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
      }
    }
  });
});
