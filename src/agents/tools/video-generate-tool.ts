...
const VideoGenerateToolSchema = Type.Object({
  ...
  durationSeconds: Type.Optional(
    Type.Number({
      description:
        "Optional target duration in seconds. WineryClaw may round this to the nearest provider-supported duration.",
      minimum: 1,
    }),
  ),
  audio: Type.Optional(
...
export function createVideoGenerateTool(options?: {
  ...
  const cfg: WineryClawConfig = options?.config ?? loadConfig();
  ...
  return {
    ...
    description:
      "Generate videos using configured providers. Generated videos are saved under WineryClaw-managed media storage and delivered automatically as attachments. Duration requests may be rounded to the nearest provider-supported value.",
