...
function isLikelyWineryClawCliProcess(argv: string[] = process.argv): boolean {
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real WineryClaw CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureWineryClawModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyWineryClawCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): WineryClawConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureWineryClawModelsJson(cfg);
    } catch {
