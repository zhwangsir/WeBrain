...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; WineryClaw handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `WineryClaw docs: ${docsPath}`,
    "Mirror: https://docs.openclaw.ai",
    "Source: https://github.com/openclaw/openclaw",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.ai",
    "For WineryClaw behavior, commands, config, or architecture: consult local docs first.",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running WineryClaw process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List WineryClaw agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List WineryClaw agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside WineryClaw.";
  }

  const lines = [
    "You are a personal assistant running inside WineryClaw.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control WineryClaw's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    ...safetySection,
    "## WineryClaw CLI Quick Reference",
    "WineryClaw is controlled via subcommands. Do not invent commands.",
    ...
    hasGateway && !isMinimal ? "## WineryClaw Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, WineryClaw pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by WineryClaw and included below in Project Context.",
    "",
