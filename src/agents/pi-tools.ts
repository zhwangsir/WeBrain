...
import { createWineryClawTools } from "./openclaw-tools.js";
...
function resolveExecConfig(params: { cfg?: WineryClawConfig; agentId?: string }) {
...
export function createWineryClawCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createWineryClawReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
