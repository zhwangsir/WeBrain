...
describe("sanitizeToolCallInputs redacts sessions_spawn attachments", () => {
  it("replaces attachments[].content with __WINERYCLAW_REDACTED__", () => {
    ...
    expect(tool?.arguments?.attachments?.[0]?.content).toBe("__WINERYCLAW_REDACTED__");
  ...
  it("replaces non-content attachment payload fields with a minimal redacted stub", () => {
    ...
    expect(attachment).toEqual({
      ...
      mimeType: "application/json",
      encoding: "utf8",
      content: "__WINERYCLAW_REDACTED__",
    });
    expect(JSON.stringify(out)).not.toContain(secret);
  });
});
