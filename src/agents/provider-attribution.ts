...
const WINERYCLAW_ATTRIBUTION_PRODUCT = "WineryClaw";
const WINERYCLAW_ATTRIBUTION_ORIGINATOR = "openclaw";
...
function formatWineryClawUserAgent(version: string): string {
  return `${WINERYCLAW_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: WINERYCLAW_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: WINERYCLAW_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatWineryClawUserAgent(identity.version),
    },
  };
}
