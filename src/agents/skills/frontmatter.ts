...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseWineryClawManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyWineryClawManifestInstallCommonFields<SkillInstallSpec>(
    {
...
): WineryClawSkillMetadata | undefined {
