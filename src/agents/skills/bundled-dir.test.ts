...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["WINERYCLAW_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns WINERYCLAW_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-bundled-override-"));
    process.env.WINERYCLAW_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
