...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousWineryClawAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousWineryClawAgentDir = process.env.WINERYCLAW_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousWineryClawAgentDir === undefined) {
      delete process.env.WINERYCLAW_AGENT_DIR;
    } else {
      process.env.WINERYCLAW_AGENT_DIR = previousWineryClawAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "WINERYCLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.WINERYCLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.WINERYCLAW_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.WINERYCLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: WineryClawConfig = {
  models: {
    providers: {
      "custom-proxy": {
