...
type PluginConfig = NonNullable<WineryClawConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: WineryClawConfig["channels"];
}): WineryClawConfig {
...
function createSinglePluginWithEmptySlotsConfig(): WineryClawConfig {
...
function createSingleNpmInstallConfig(installPath: string): WineryClawConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as WineryClawConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
