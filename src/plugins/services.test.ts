...
import type { WineryClawPluginService, WineryClawPluginServiceContext } from "./types.js";
...
function createRegistry(services: WineryClawPluginService[]) {
...
function expectServiceContext(
  ctx: WineryClawPluginServiceContext,
...
function expectServiceLogger(ctx: WineryClawPluginServiceContext) {
...
function expectServiceContexts(
  contexts: WineryClawPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
async function startTrackingServices(params: {
  services: WineryClawPluginService[];
  config?: Parameters<typeof startPluginServices>[0]["config"];
  workspaceDir?: string;
}) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: WineryClawPluginServiceContext[] = [];
