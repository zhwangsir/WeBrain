...
describe("shouldAttemptTtsPayload", () => {
  ...
  beforeEach(() => {
    originalPrefsPath = process.env.WINERYCLAW_TTS_PREFS;
    dir = mkdtempSync(path.join(tmpdir(), "openclaw-tts-config-"));
    prefsPath = path.join(dir, "tts.json");
    process.env.WINERYCLAW_TTS_PREFS = prefsPath;
  });

  afterEach(() => {
    if (originalPrefsPath === undefined) {
      delete process.env.WINERYCLAW_TTS_PREFS;
    } else {
      process.env.WINERYCLAW_TTS_PREFS = originalPrefsPath;
    }
    rmSync(dir, { recursive: true, force: true });
  });

  it("skips TTS when config, prefs, and session state leave auto mode off", () => {
    expect(shouldAttemptTtsPayload({ cfg: {} as WineryClawConfig })).toBe(false);
  });

  it("honors session auto state before prefs and config", () => {
    writeFileSync(prefsPath, JSON.stringify({ tts: { auto: "off" } }));
    const cfg = { messages: { tts: { auto: "off" } } } as WineryClawConfig;
  ...
  it("uses local prefs before config auto mode", () => {
    ...
    writeFileSync(prefsPath, JSON.stringify({ tts: { auto: "off" } }));
    expect(
      shouldAttemptTtsPayload({ cfg: { messages: { tts: { enabled: true } } } as WineryClawConfig }),
    ).toBe(false);
  });
});
