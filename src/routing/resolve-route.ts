...
function listAgents(cfg: WineryClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: WineryClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<WineryClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: WineryClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: WineryClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: WineryClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<WineryClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  WineryClawConfig,
  {
    bindingsRef: WineryClawConfig["bindings"];
    agentsRef: WineryClawConfig["agents"];
    sessionRef: WineryClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: WineryClawConfig): Map<string, ResolvedAgentRoute> {
