const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "WINERYCLAW_LAUNCHD_LABEL"],
  systemd: ["WINERYCLAW_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["WINERYCLAW_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "WINERYCLAW_SERVICE_MARKER",
  "WINERYCLAW_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.WINERYCLAW_SERVICE_MARKER?.trim();
    const serviceKind = env.WINERYCLAW_SERVICE_KIND?.trim();
