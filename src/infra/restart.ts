...
export function triggerWineryClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.WINERYCLAW_SYSTEMD_UNIT,
      process.env.WINERYCLAW_PROFILE,
    );
  ...
  const label =
    process.env.WINERYCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.WINERYCLAW_PROFILE);
