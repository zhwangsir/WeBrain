...
const DEFAULT_HOST_PATH = "~/.wineryclaw/exec-approvals.json";
...
function formatRequestedSource(params: {
  ...
  return params.sourcePath === "__default__"
    ? `WineryClaw default (${params.defaultValue})`
...
function formatHostFieldSource(params: {
  ...
  if (params.field === "askFallback") {
    return `WineryClaw default (${DEFAULT_EXEC_APPROVAL_ASK_FALLBACK})`;
  }
