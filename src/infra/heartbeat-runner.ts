...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: WineryClawConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: WineryClawConfig) {
...
function resolveHeartbeatAgents(cfg: WineryClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: WineryClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: WineryClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: WineryClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
