...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    const prevWineryClawHome = process.env.WINERYCLAW_HOME;

    try {
      process.env.WINERYCLAW_HOME = dir;
      const approvalsPath = path.join(dir, ".wineryclaw", "exec-approvals.json");
      ...
      if (prevWineryClawHome === undefined) {
        delete process.env.WINERYCLAW_HOME;
      } else {
        process.env.WINERYCLAW_HOME = prevWineryClawHome;
      }
    }
  });
});
