...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: WineryClawConfig | null = null;
  ...
  get(key: string, cfg: WineryClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: WineryClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: WineryClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: WineryClawConfig): void {
