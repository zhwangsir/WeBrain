...
WINERYCLAW_DOCKER_BUILD_USE_BUILDX="${WINERYCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
WINERYCLAW_DOCKER_BUILD_CACHE_FROM="${WINERYCLAW_DOCKER_BUILD_CACHE_FROM:-}"
WINERYCLAW_DOCKER_BUILD_CACHE_TO="${WINERYCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${WINERYCLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${WINERYCLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${WINERYCLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${WINERYCLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${WINERYCLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
