...
# Build and bundle WineryClaw into a minimal .app we can open.
# Outputs to dist/WineryClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/WineryClaw.app"
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/WineryClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :WineryClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :WineryClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/WineryClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/WineryClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/WineryClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/WineryClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/WineryClaw/Resources/WineryClaw.icns" "$APP_ROOT/Contents/Resources/WineryClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/WineryClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying WineryClawKit resources"
WINERYCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/WineryClawKit_WineryClawKit.bundle"
if [ -d "$WINERYCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/WineryClawKit_WineryClawKit.bundle"
  cp -R "$WINERYCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/WineryClawKit_WineryClawKit.bundle"
else
  echo "WARN: WineryClawKit resource bundle not found at $WINERYCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running WineryClaw"
killall -q WineryClaw 2>/dev/null || true
