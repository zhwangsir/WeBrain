...
DEFAULT_PROVIDER="${WINERYCLAW_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
CLI_MODEL="${WINERYCLAW_LIVE_CLI_BACKEND_MODEL:-}"
CLI_PROVIDER="${CLI_MODEL%%/*}"
CLI_DISABLE_MCP_CONFIG="${WINERYCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG:-}"
CLI_AUTH_MODE="${WINERYCLAW_LIVE_CLI_BACKEND_AUTH:-auto}"
...
case "$CLI_AUTH_MODE" in
  auto | api-key | subscription)
    ;;
  *)
    echo "ERROR: WINERYCLAW_LIVE_CLI_BACKEND_AUTH must be one of: auto, api-key, subscription." >&2
    exit 1
    ;;
esac

if [[ "$CLI_AUTH_MODE" == "subscription" && "$CLI_PROVIDER" != "claude-cli" ]]; then
  echo "ERROR: WINERYCLAW_LIVE_CLI_BACKEND_AUTH=subscription is only supported for claude-cli." >&2
  exit 1
fi
...
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  ...
  if [[ -z "${WINERYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]]; then
    if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" ]]; then
      export WINERYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV='["CLAUDE_CODE_OAUTH_TOKEN"]'
    else
      export WINERYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="[]"
    fi
  fi
  if [[ "$WINERYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV" == *ANTHROPIC_API_KEY* ]]; then
    echo "ERROR: subscription auth smoke must not preserve Anthropic API-key env vars." >&2
    exit 1
  fi
  if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" && "$WINERYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV" != *CLAUDE_CODE_OAUTH_TOKEN* ]]; then
  ...
  export WINERYCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${WINERYCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
  export WINERYCLAW_LIVE_CLI_BACKEND_RESUME_PROBE="${WINERYCLAW_LIVE_CLI_BACKEND_RESUME_PROBE:-1}"
  export WINERYCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${WINERYCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
  export WINERYCLAW_LIVE_CLI_BACKEND_MCP_PROBE="${WINERYCLAW_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
fi
...
provider="${WINERYCLAW_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
default_command="${WINERYCLAW_DOCKER_CLI_BACKEND_COMMAND_DEFAULT:-}"
docker_package="${WINERYCLAW_DOCKER_CLI_BACKEND_NPM_PACKAGE:-}"
binary_name="${WINERYCLAW_DOCKER_CLI_BACKEND_BINARY_NAME:-}"
if [ -z "$binary_name" ] && [ -n "$default_command" ]; then
  binary_name="$(basename "$default_command")"
fi
if [ -z "${WINERYCLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -n "$binary_name" ]; then
  export WINERYCLAW_LIVE_CLI_BACKEND_COMMAND="$HOME/.npm-global/bin/$binary_name"
fi
if [ -n "${WINERYCLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ ! -x "${WINERYCLAW_LIVE_CLI_BACKEND_COMMAND}" ] && [ -n "$docker_package" ]; then
  npm_config_prefix="$HOME/.npm-global" npm install -g "$docker_package"
fi
if [ "$provider" = "claude-cli" ]; then
  auth_mode="${WINERYCLAW_LIVE_CLI_BACKEND_AUTH:-auto}"
...
if [ -n "\${WINERYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${WINERYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${WINERYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${WINERYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  if [ -z "${WINERYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]; then
    export WINERYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'
  fi
  if [ "$auth_mode" = "subscription" ]; then
    claude --version
    direct_token="WINERYCLAW-CLAUDE-SUBSCRIPTION-DIRECT"
    direct_output="$(
      claude \
...
if [[ "${WINERYCLAW_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (WINERYCLAW_SKIP_DOCKER_BUILD=1)"
else
...
DOCKER_AUTH_ENV=(
  -e WINERYCLAW_LIVE_CLI_BACKEND_AUTH="$CLI_AUTH_MODE"
)
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  DOCKER_AUTH_ENV+=(
    -e CLAUDE_CODE_OAUTH_TOKEN="${CLAUDE_CODE_OAUTH_TOKEN:-}"
    -e WINERYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="$WINERYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV"
  )
else
  DOCKER_AUTH_ENV+=(
    -e ANTHROPIC_API_KEY
    -e ANTHROPIC_API_KEY_OLD
    -e WINERYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
    -e WINERYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}"
    -e WINERYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="${WINERYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}"
  )
fi
...
docker run --rm -t \
  ...
  -e WINERYCLAW_DOCKER_CLI_BACKEND_PROVIDER="$CLI_PROVIDER" \
  -e WINERYCLAW_DOCKER_CLI_BACKEND_COMMAND_DEFAULT="$CLI_DEFAULT_COMMAND" \
  -e WINERYCLAW_DOCKER_CLI_BACKEND_NPM_PACKAGE="$CLI_DOCKER_NPM_PACKAGE" \
  -e WINERYCLAW_DOCKER_CLI_BACKEND_BINARY_NAME="$CLI_DOCKER_BINARY_NAME" \
  -e WINERYCLAW_LIVE_TEST=1 \
  -e WINERYCLAW_LIVE_CLI_BACKEND=1 \
  -e WINERYCLAW_LIVE_CLI_BACKEND_DEBUG="${WINERYCLAW_LIVE_CLI_BACKEND_DEBUG:-}" \
  -e WINERYCLAW_CLI_BACKEND_LOG_OUTPUT="${WINERYCLAW_CLI_BACKEND_LOG_OUTPUT:-}" \
  -e WINERYCLAW_LIVE_CLI_BACKEND_MODEL="$CLI_MODEL" \
  -e WINERYCLAW_LIVE_CLI_BACKEND_COMMAND="${WINERYCLAW_LIVE_CLI_BACKEND_COMMAND:-}" \
  -e WINERYCLAW_LIVE_CLI_BACKEND_ARGS="${WINERYCLAW_LIVE_CLI_BACKEND_ARGS:-}" \
  -e WINERYCLAW_LIVE_CLI_BACKEND_CLEAR_ENV="${WINERYCLAW_LIVE_CLI_BACKEND_CLEAR_ENV:-}" \
  -e WINERYCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG="$CLI_DISABLE_MCP_CONFIG" \
  -e WINERYCLAW_LIVE_CLI_BACKEND_RESUME_PROBE="${WINERYCLAW_LIVE_CLI_BACKEND_RESUME_PROBE:-}" \
  -e WINERYCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${WINERYCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-}" \
  -e WINERYCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${WINERYCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-}" \
  -e WINERYCLAW_LIVE_CLI_BACKEND_MCP_PROBE="${WINERYCLAW_LIVE_CLI_BACKEND_MCP_PROBE:-}" \
  -e WINERYCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="${WINERYCLAW_LIVE_CLI_BACKEND_IMAGE_ARG:-}" \
  -e WINERYCLAW_LIVE_CLI_BACKEND_IMAGE_MODE="${WINERYCLAW_LIVE_CLI_BACKEND_IMAGE_MODE:-}" \
  -v "$ROOT_DIR":/src:ro \
