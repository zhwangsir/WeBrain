...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${WINERYCLAW_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$WINERYCLAW_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
SMOKE_IMAGE="${WINERYCLAW_INSTALL_SMOKE_IMAGE:-openclaw-install-smoke:local}"
NONROOT_IMAGE="${WINERYCLAW_INSTALL_NONROOT_IMAGE:-openclaw-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${WINERYCLAW_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${WINERYCLAW_INSTALL_URL:-https://openclaw.bot/install.sh}"
CLI_INSTALL_URL="${WINERYCLAW_INSTALL_CLI_URL:-https://openclaw.bot/install-cli.sh}"
PACKAGE_NAME="${WINERYCLAW_INSTALL_PACKAGE:-openclaw}"
SKIP_NONROOT="${WINERYCLAW_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${WINERYCLAW_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${WINERYCLAW_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${WINERYCLAW_INSTALL_SMOKE_SKIP_UPDATE:-0}"
UPDATE_BASELINE_VERSION="${WINERYCLAW_INSTALL_SMOKE_UPDATE_BASELINE:-2026.4.10}"
UPDATE_PACKAGE_SPEC="${WINERYCLAW_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_SKIP_LOCAL_BUILD="${WINERYCLAW_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${WINERYCLAW_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${WINERYCLAW_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${WINERYCLAW_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (WINERYCLAW_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  start_update_server
  ...
  docker run --rm -t \
    ...
    -e WINERYCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e WINERYCLAW_INSTALL_METHOD=npm \
    -e WINERYCLAW_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e WINERYCLAW_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e WINERYCLAW_INSTALL_LATEST_OUT="/out/latest" \
    -e WINERYCLAW_NO_ONBOARD=1 \
    -e WINERYCLAW_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${UPDATE_DOCKER_HOST_ARGS[@]}" \
    -e WINERYCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e WINERYCLAW_INSTALL_SMOKE_MODE=update \
    -e WINERYCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e WINERYCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e WINERYCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e WINERYCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e WINERYCLAW_NO_ONBOARD=1 \
    -e WINERYCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (WINERYCLAW_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e WINERYCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e WINERYCLAW_INSTALL_METHOD=npm \
    -e WINERYCLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e WINERYCLAW_NO_ONBOARD=1 \
    -e WINERYCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${WINERYCLAW_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (WINERYCLAW_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e WINERYCLAW_INSTALL_URL="$INSTALL_URL" \
  -e WINERYCLAW_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e WINERYCLAW_NO_ONBOARD=1 \
  -e WINERYCLAW_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
