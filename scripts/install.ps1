# WineryClaw Installer for Windows (PowerShell)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 WineryClaw Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one WineryClaw.$NC" -Level info
    Write-Host ""
}
...
function Install-WineryClawGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing WineryClaw from git..." -Level info
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install WineryClaw from git to $GitDir" -Level info
        } else {
            Install-WineryClawGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install WineryClaw via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-WineryClawNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    Write-Host ""
    Write-Host "🦞 WineryClaw installed successfully!" -Level success
}
