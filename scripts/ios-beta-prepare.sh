...
- regenerates apps/ios/WineryClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${WINERYCLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid WINERYCLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid WINERYCLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid WINERYCLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid WINERYCLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing WINERYCLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(WINERYCLAW_URL_SLASH)$(WINERYCLAW_URL_SLASH)#g'
)"
...
WINERYCLAW_CODE_SIGN_STYLE = Automatic
WINERYCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
WINERYCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
WINERYCLAW_APP_BUNDLE_ID = ai.openclaw.client
WINERYCLAW_SHARE_BUNDLE_ID = ai.openclaw.client.share
WINERYCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.openclaw.client.activitywidget
WINERYCLAW_WATCH_APP_BUNDLE_ID = ai.openclaw.client.watchkitapp
WINERYCLAW_WATCH_EXTENSION_BUNDLE_ID = ai.openclaw.client.watchkitapp.extension
WINERYCLAW_APP_PROFILE =
WINERYCLAW_SHARE_PROFILE =
WINERYCLAW_PUSH_TRANSPORT = relay
WINERYCLAW_PUSH_DISTRIBUTION = official
WINERYCLAW_URL_SLASH = /
WINERYCLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
WINERYCLAW_PUSH_APNS_ENVIRONMENT = production
EOF
