#!/usr/bin/env bash
# Rootless WineryClaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-wineryclaw-podman.sh launch        # Start gateway
#   ./scripts/run-wineryclaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   wineryclaw --container wineryclaw dashboard --no-open
#   wineryclaw --container wineryclaw channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      WINERYCLAW_GATEWAY_TOKEN|WINERYCLAW_PODMAN_CONTAINER|WINERYCLAW_PODMAN_IMAGE|WINERYCLAW_IMAGE|WINERYCLAW_PODMAN_PULL|WINERYCLAW_PODMAN_GATEWAY_HOST_PORT|WINERYCLAW_GATEWAY_PORT|WINERYCLAW_PODMAN_BRIDGE_HOST_PORT|WINERYCLAW_BRIDGE_PORT|WINERYCLAW_GATEWAY_BIND|WINERYCLAW_PODMAN_USERNS|WINERYCLAW_BIND_MOUNT_OPTIONS|WINERYCLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-wineryclaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${WINERYCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.openclaw}"
ENV_FILE="${WINERYCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${WINERYCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.openclaw}"
ENV_FILE="${WINERYCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${WINERYCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${WINERYCLAW_PODMAN_CONTAINER:-wineryclaw}"
WINERYCLAW_IMAGE="${WINERYCLAW_PODMAN_IMAGE:-${WINERYCLAW_IMAGE:-wineryclaw:local}}"
PODMAN_PULL="${WINERYCLAW_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${WINERYCLAW_PODMAN_GATEWAY_HOST_PORT:-${WINERYCLAW_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${WINERYCLAW_PODMAN_BRIDGE_HOST_PORT:-${WINERYCLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${WINERYCLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$WINERYCLAW_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v wineryclaw >/dev/null 2>&1; then
    return 0
  fi
  WINERYCLAW_CONTAINER="" WINERYCLAW_CONFIG_DIR="$config_dir" \
    wineryclaw config get gateway.bind 2>/dev/null || true
}
...
# WINERYCLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${WINERYCLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'WINERYCLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v wineryclaw >/dev/null 2>&1; then
    echo "Warning: wineryclaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    WINERYCLAW_CONTAINER="" WINERYCLAW_CONFIG_DIR="$config_dir" \
      wineryclaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    WINERYCLAW_CONTAINER="" WINERYCLAW_CONFIG_DIR="$config_dir" \
      wineryclaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  WINERYCLAW_CONTAINER="" WINERYCLAW_CONFIG_DIR="$config_dir" \
    wineryclaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${WINERYCLAW_GATEWAY_TOKEN:-}" ]]; then
  export WINERYCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "WINERYCLAW_GATEWAY_TOKEN" "$WINERYCLAW_GATEWAY_TOKEN"
  echo "Generated WINERYCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/wineryclaw.json"
...
PODMAN_USERNS="${WINERYCLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported WINERYCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (WINERYCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${WINERYCLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${WINERYCLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$WINERYCLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.openclaw/.npm \
    -e WINERYCLAW_NO_RESPAWN=1 \
...
echo "podman exec -it $CONTAINER_NAME wineryclaw dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME wineryclaw devices approve --latest  # if pairing required"
