...
- WINERYCLAW_IOS_VERSION = exact canonical iOS version
- WINERYCLAW_MARKETING_VERSION = short iOS/App Store version
- WINERYCLAW_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    WINERYCLAW_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    WINERYCLAW_MARKETING_VERSION)
...
WINERYCLAW_IOS_VERSION = ${IOS_VERSION}
WINERYCLAW_MARKETING_VERSION = ${MARKETING_VERSION}
WINERYCLAW_BUILD_VERSION = ${BUILD_NUMBER}
EOF
