---
summary: "Run WineryClaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want WineryClaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# WineryClaw on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent WineryClaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "WineryClaw 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-openclaw-project --name="WineryClaw Gateway"
  ...
  <Step title="Configure environment variables">
    ...
    WINERYCLAW_CONFIG_DIR=/home/$USER/.openclaw
    WINERYCLAW_WORKSPACE_DIR=/home/$USER/.wineryclaw/workspace
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`WINERYCLAW_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:
...
1. Create a service account:
   ...
   gcloud iam service-accounts create openclaw-deploy \
     --display-name="WineryClaw Deployment"
   ```
