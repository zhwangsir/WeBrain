---
summary: "Deploy WineryClaw on Render with Infrastructure-as-Code"
read_when:
  - Deploying WineryClaw to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy WineryClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
repository configures everything needed to run WineryClaw:
...
your fork's `render.yaml` (but note: no persistent disk means WineryClaw state
...
If you use the original WineryClaw repository, Render will not auto-deploy your WineryClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For WineryClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with WineryClaw state plus any configured
