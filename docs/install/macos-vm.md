---
summary: "Run WineryClaw in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want WineryClaw isolated from your main macOS environment
...
# WineryClaw on macOS VMs (Sandboxing)
...
Run WineryClaw in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
5. SSH in, install WineryClaw, configure channels
6. Done
...
```bash
nano ~/.wineryclaw/wineryclaw.json
```
...
The VM runs in the background. WineryClaw's daemon keeps the gateway running.
...
This is the killer feature of running on macOS. Use [BlueBubbles](https://bluebubbles.app) to add iMessage to WineryClaw.
