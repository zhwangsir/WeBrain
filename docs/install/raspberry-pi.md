---
summary: "Host WineryClaw on a Raspberry Pi for always-on self-hosting"
read_when:
...
Run a persistent, always-on WineryClaw Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well.
