---
summary: "Run WineryClaw in a rootless Podman container"
read_when:
...
Run the WineryClaw Gateway in a rootless Podman container, managed by your current non-root user.
...
- **WineryClaw CLI** installed on the host
...
<Steps>
  ...
  <Step title="Manage the running container from the host CLI">
    Set `WINERYCLAW_CONTAINER=openclaw`, then use normal `openclaw` commands from the host.
  </Step>
</Steps>

Setup details:

- `./scripts/podman/setup.sh` builds `openclaw:local` in your rootless Podman store by default, or uses `WINERYCLAW_IMAGE` / `WINERYCLAW_PODMAN_IMAGE` if you set one.
- It creates `~/.wineryclaw/wineryclaw.json` with `gateway.mode: "local"` if missing.
- It creates `~/.wineryclaw/.env` with `WINERYCLAW_GATEWAY_TOKEN` if missing.
- For manual launches, the helper reads only a small allowlist of Podman-related keys from `~/.wineryclaw/.env` and passes explicit runtime env vars to the container; it does not hand the full env file to Podman.
...
You can also set `WINERYCLAW_PODMAN_QUADLET=1`.

Optional build/setup env vars:

- `WINERYCLAW_IMAGE` or `WINERYCLAW_PODMAN_IMAGE` -- use an existing/pulled image instead of building `openclaw:local`
- `WINERYCLAW_DOCKER_APT_PACKAGES` -- install extra apt packages during image build
- `WINERYCLAW_EXTENSIONS` -- pre-install extension dependencies at build time
...
The script starts the container as your current uid/gid with `--userns=keep-id` and bind-mounts your WineryClaw state into the container.
...
Then open `http://127.0.0.1:18789/` and use the token from `~/.wineryclaw/.env`.

Host CLI default:

```bash
export WINERYCLAW_CONTAINER=openclaw
```
...
- **Config dir:** `~/.openclaw`
- **Workspace dir:** `~/.wineryclaw/workspace`
- **Token file:** `~/.wineryclaw/.env`
...
`wineryclaw.json`, per-agent `auth-profiles.json`, channel/provider state,
...
- `WINERYCLAW_PODMAN_CONTAINER` -- container name (`openclaw` by default)
- `WINERYCLAW_PODMAN_IMAGE` / `WINERYCLAW_IMAGE` -- image to run
- `WINERYCLAW_PODMAN_GATEWAY_HOST_PORT` -- host port mapped to container `18789`
- `WINERYCLAW_PODMAN_BRIDGE_HOST_PORT` -- host port mapped to container `18790`
- `WINERYCLAW_PODMAN_PUBLISH_HOST` -- host interface for published ports; default is `127.0.0.1`
- `WINERYCLAW_GATEWAY_BIND` -- gateway bind mode inside the container; default is `lan`
- `WINERYCLAW_PODMAN_USERNS` -- `keep-id` (default), `auto`, or `host`

The manual launcher reads `~/.wineryclaw/.env` before finalizing container/image defaults, so you can persist these there.

If you use a non-default `WINERYCLAW_CONFIG_DIR` or `WINERYCLAW_WORKSPACE_DIR`, set the same variables for both `./scripts/podman/setup.sh` and later `./scripts/run-openclaw-podman.sh launch` commands. The repo-local launcher does not persist custom path overrides across shells.
...
- It pins `WINERYCLAW_NO_RESPAWN=1`, `Restart=on-failure`, and `TimeoutStartSec=300`.
- It publishes both `127.0.0.1:18789:18789` (gateway) and `127.0.0.1:18790:18790` (bridge).
- It reads `~/.wineryclaw/.env` as a runtime `EnvironmentFile` for values such as `WINERYCLAW_GATEWAY_TOKEN`, but it does not consume the manual launcher's Podman-specific override allowlist.
...
- **Gateway start blocked (missing `gateway.mode=local`):** Ensure `~/.wineryclaw/wineryclaw.json` exists and sets `gateway.mode="local"`. `scripts/podman/setup.sh` creates this if missing.
- **Container CLI commands hit the wrong target:** Use `openclaw --container <name> ...` explicitly, or export `WINERYCLAW_CONTAINER=<name>` in your shell.
