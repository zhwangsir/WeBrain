...
read_when:
  - Setting up Twitch chat integration for WineryClaw
title: "Twitch"
---

# Twitch

Twitch chat support via IRC connection. WineryClaw connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

Twitch ships as a bundled plugin in current WineryClaw releases, so normal
...
5. Configure the token:
   - Env: `WINERYCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      ...
      accessToken: "oauth:abc123...", // OAuth Access Token (or use WINERYCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
WINERYCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
