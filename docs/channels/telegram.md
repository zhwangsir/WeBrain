...
- DM messages can carry `message_thread_id`; WineryClaw routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    WineryClaw can stream partial replies in real time:
    ...
    - DM: WineryClaw keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: WineryClaw keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), WineryClaw falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, WineryClaw skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, WineryClaw automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, WineryClaw retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - WineryClaw pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.wineryclaw/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, WineryClaw enqueues system events like:
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, WineryClaw preserves the topic for both the approval prompt and the post-approval follow-up. Exec approvals expire after 30 minutes by default.
...
<AccordionGroup>
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, WineryClaw now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      ...
      - `WINERYCLAW_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- Multi-account precedence:
  ...
  - If neither is set, WineryClaw falls back to the first normalized account ID and `openclaw doctor` warns.
