---
summary: "Synology Chat webhook setup and WineryClaw config"
read_when:
  - Setting up Synology Chat with WineryClaw
...
Synology Chat ships as a bundled plugin in current WineryClaw releases, so normal
...
3. Point the outgoing webhook URL to your WineryClaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in WineryClaw.
...
- WineryClaw accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
```bash
openclaw message send --channel synology-chat --target 123456 --text "Hello from WineryClaw"
...
Give each enabled account a distinct `webhookPath`. WineryClaw now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached WineryClaw
- `Rate limit exceeded`:
