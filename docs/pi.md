...
summary: "Architecture of WineryClaw's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in WineryClaw
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how WineryClaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

WineryClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, WineryClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in WineryClaw's local TUI mode)                                             |
...
├── pi-tools.ts                    # createWineryClawCodingTools()
...
├── openclaw-tools.ts              # WineryClaw-specific tools
...
Image injection is prompt-local: WineryClaw loads image refs from the current prompt and
...
2. **Custom Replacements**: WineryClaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **WineryClaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
WineryClaw wraps this with `guardSessionManager()` for tool result safety.
...
WineryClaw maintains an auth profile store with multiple API keys per provider:
...
WineryClaw loads custom pi extensions for specialized behavior:
...
WineryClaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | WineryClaw Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom WineryClaw tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.wineryclaw/agents/<agentId>/sessions/` (or `$WINERYCLAW_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `WINERYCLAW_LIVE_TEST=1`)
