...
For shared provider-backed agent sessions, WineryClaw starts music generation as a
...
Not all providers support all parameters. WineryClaw still validates hard limits
such as input counts before submission. When a provider supports duration but
uses a shorter maximum than the requested value, WineryClaw automatically clamps
...
Tool results report the applied settings. When WineryClaw clamps duration during provider fallback, the returned `durationSeconds` reflects the submitted value and `details.normalization.durationSeconds` shows the requested-to-applied mapping.
...
- Completion wake: WineryClaw injects an internal completion event back into the same session so the model can write the user-facing follow-up itself.
...
When generating music, WineryClaw tries providers in this order:
...
```bash
WINERYCLAW_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
