---
summary: "Use ACP runtime sessions for Codex, Claude Code, Cursor, Gemini CLI, WineryClaw ACP, and other harness agents"
read_when:
...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let WineryClaw run external coding harnesses (for example Pi, Claude Code, Codex, Cursor, Copilot, WineryClaw ACP, OpenCode, Gemini CLI, and other supported ACPX harnesses) through an ACP backend plugin.

If you ask WineryClaw in plain language to "run this in Codex" or "start Claude Code in a thread", WineryClaw should route that request to the ACP runtime (not the native sub-agent runtime). Each ACP session spawn is tracked as a [background task](/automation/tasks).

If you want Codex or Claude Code to connect as an external MCP client directly
to existing WineryClaw channel conversations, use [`openclaw mcp serve`](/cli/mcp)
instead of ACP.
...
| ---------------------------------------------------------------------------------- | ------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Run Codex, Claude Code, Gemini CLI, or another external harness _through_ WineryClaw | This page: ACP agents                 | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls |
| Expose an WineryClaw Gateway session _as_ an ACP server for an editor or client      | [`openclaw acp`](/cli/acp)            | Bridge mode. IDE/client talks ACP to WineryClaw over stdio/WebSocket                                          |
| Reuse a local AI CLI as a text-only fallback model                                 | [CLI Backends](/gateway/cli-backends) | Not ACP. No WineryClaw tools, no ACP controls, no harness runtime                                             |
...
- On startup, WineryClaw probes that binary and self-repairs it if needed.
...
- `/acp spawn codex`: WineryClaw should be ready to bootstrap `acpx`, but the Codex ACP adapter may still need a first-run fetch.
...
Use ACP when you want an external harness runtime. Use sub-agents when you want WineryClaw-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | WineryClaw native sub-agent runtime  |
...
1. WineryClaw ACP session control plane
...
- WineryClaw keeps owning the channel transport, auth, safety, and delivery.
...
- If you spawn to a different ACP agent and do not pass `--cwd`, WineryClaw inherits the **target agent's** workspace by default, not the requester's.
- If that inherited workspace path is missing (`ENOENT`/`ENOTDIR`), WineryClaw falls back to the backend default cwd instead of silently reusing the wrong tree.
...
- ACP session: the durable Codex/Claude/Gemini runtime state WineryClaw routes to
...
- `/acp spawn codex --thread auto`: WineryClaw may create a child thread/topic and bind the ACP session there
...
- Generic current-conversation binds use the shared WineryClaw binding store and survive normal gateway restarts.

Notes:

- `--bind here` and `--thread ...` are mutually exclusive on `/acp spawn`.
- On Discord, `--bind here` binds the current channel or thread in place. `spawnAcpSessions` is only required when WineryClaw needs to create a child thread for `--thread auto|here`.
- If the active channel does not expose current-conversation ACP bindings, WineryClaw returns a clear unsupported message.
...
- WineryClaw binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, WineryClaw returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning WineryClaw agent id.
...
- WineryClaw ensures the configured ACP session exists before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, WineryClaw inherits the target agent workspace from agent config.
...
- If `agentId` is omitted, WineryClaw uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, WineryClaw may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new WineryClaw session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the WineryClaw sandbox.
...
If no target resolves, WineryClaw returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, WineryClaw returns a clear unsupported-control error.
...
When WineryClaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal WineryClaw `agentId` path).
...
- Relative paths resolve from WineryClaw workspace directory.
...
- WineryClaw startup remains non-blocking while the backend health check runs.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install WineryClaw globally with `npm install -g openclaw`, the acpx
...
By default, ACPX sessions do **not** expose WineryClaw plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
WineryClaw plugin tools such as memory recall/store, enable the dedicated bridge:
...
- Injects a built-in MCP server named `openclaw-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled WineryClaw
  plugins.
...
These ACPX harness permissions are separate from WineryClaw exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
> **Important:** WineryClaw currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
