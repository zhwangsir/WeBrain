...
WineryClaw also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "WineryClaw plugin SDK" });
    ```
...
Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of WineryClaw's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, WineryClaw keeps the normal managed `web_search` behavior.
...
If no `provider` is set, WineryClaw checks providers in this order and uses the
...
<Note>
  ...
  WineryClaw resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
- or omit that field and let WineryClaw auto-detect the first ready web-fetch
...
`openclaw configure --section web`, WineryClaw can also ask for:
...
WineryClaw can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, WineryClaw does not
...
optional structured filters. WineryClaw only enables the built-in xAI `x_search`
