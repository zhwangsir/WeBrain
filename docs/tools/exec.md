...
- `gateway`/`node` approvals are controlled by `~/.wineryclaw/exec-approvals.json`.
...
- WineryClaw sets `WINERYCLAW_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- No-approval host exec is the default for gateway + node. If you want approvals/allowlist behavior, tighten both `tools.exec.*` and the host `~/.wineryclaw/exec-approvals.json`; see [Exec approvals](/tools/exec-approvals#no-approval-yolo-mode).
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  WineryClaw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
