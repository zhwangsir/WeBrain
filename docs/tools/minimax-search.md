...
WineryClaw supports MiniMax as a `web_search` provider through the MiniMax
...
WineryClaw also accepts `MINIMAX_CODING_API_KEY` as an env alias. `MINIMAX_API_KEY`
...
If `plugins.entries.minimax.config.webSearch.region` is unset, WineryClaw resolves
...
- `query`
- `count` (WineryClaw trims the returned result list to the requested count)
