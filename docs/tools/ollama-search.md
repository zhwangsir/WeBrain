...
- an Ollama host that is reachable from WineryClaw
- `ollama signin`
...
If no explicit Ollama base URL is set, WineryClaw uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, WineryClaw reuses
...
- If the Ollama host is auth-protected, WineryClaw reuses the normal Ollama
  provider API key when present.
- WineryClaw warns during setup if Ollama is unreachable or not signed in, but
