...
WineryClaw supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), WineryClaw reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.wineryclaw/.env`.

If you omit `baseUrl`, WineryClaw defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, WineryClaw defaults to `kimi-k2.5`.
