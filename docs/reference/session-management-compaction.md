...
This document explains how WineryClaw manages sessions end-to-end:
...
WineryClaw is designed around a single **Gateway process** that owns session state.
...
WineryClaw persists sessions in two layers:
...
WineryClaw resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, WineryClaw reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.wineryclaw/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
WineryClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When WineryClaw splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, WineryClaw
...
- If a trailing tool-result block would otherwise push the chunk over target,
  WineryClaw preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (WineryClaw consumes the events, but Pi decides when to compact).
...
WineryClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, WineryClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, WineryClaw leaves it alone.
...
- If the provider fails or returns an empty result, WineryClaw falls back to built-in LLM summarization automatically.
...
WineryClaw supports “silent” turns for background tasks where the user should not see intermediate output.
...
- WineryClaw strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, WineryClaw also suppresses **draft/typing streaming** when a
...
WineryClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but WineryClaw’s
