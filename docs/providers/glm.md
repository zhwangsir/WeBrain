---
summary: "GLM model family overview + how to use it in WineryClaw"
read_when:
  - You want GLM models in WineryClaw
...
GLM is a **model family** (not a company) available through the Z.AI platform. In WineryClaw, GLM
...
<Tip>
`zai-api-key` lets WineryClaw detect the matching Z.AI endpoint from the key and
...
WineryClaw currently seeds the bundled `zai` provider with these GLM refs:
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, WineryClaw inspects the key format
