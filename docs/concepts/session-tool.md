...
WineryClaw gives agents tools to work across sessions, inspect status, and
...
After the target responds, WineryClaw can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned WineryClaw
...
the completion origin only identifies a channel WineryClaw can still reuse the
