...
- **Session store** (chat history + routing state) under `~/.wineryclaw/agents/<agentId>/sessions`.
...
`~/.wineryclaw/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.openclaw` (or `WINERYCLAW_STATE_DIR`)
- Workspace: `~/.wineryclaw/workspace` (or `~/.wineryclaw/workspace-<agentId>`)
- Agent dir: `~/.wineryclaw/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.wineryclaw/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, WineryClaw runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.wineryclaw/workspace` (or `~/.wineryclaw/workspace-<profile>` when `WINERYCLAW_PROFILE` is set).
- State defaults to `~/.wineryclaw/agents/main/agent`.
...
Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.wineryclaw/agents/<agentId>`.
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.wineryclaw/workspace-alex" },
      { id: "mia", workspace: "~/.wineryclaw/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, WineryClaw upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, WineryClaw falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.wineryclaw/workspace-main" },
      { id: "coding", workspace: "~/.wineryclaw/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.wineryclaw/workspace-main" },
      { id: "alerts", workspace: "~/.wineryclaw/workspace-alerts" },
    ],
  },
  bindings: [
...
`~/.wineryclaw/wineryclaw.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.wineryclaw/workspace-home",
        agentDir: "~/.wineryclaw/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.wineryclaw/workspace-work",
        agentDir: "~/.wineryclaw/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.wineryclaw/credentials/whatsapp/personal
          // authDir: "~/.wineryclaw/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.wineryclaw/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.wineryclaw/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.wineryclaw/workspace-opus",
