...
read_when:
  - Building or debugging native WineryClaw plugins
...
This page covers the internal architecture of the WineryClaw plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside WineryClaw. Every
native WineryClaw plugin registers against one or more capability types:
...
WineryClaw classifies every loaded plugin into a shape based on its actual
...
WineryClaw's plugin system has four layers:

1. **Manifest + discovery**
   WineryClaw finds candidate plugins from configured paths, workspace roots,
...
3. **Runtime loading**
   Native WineryClaw plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of WineryClaw reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting WineryClaw
...
That split lets WineryClaw validate config, explain missing/disabled plugins, and
...
normal chat actions. WineryClaw keeps one shared `message` tool in core, and
...
WineryClaw treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's WineryClaw-facing
  surfaces
...
So if WineryClaw adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If WineryClaw has shared
...
```ts
import type { WineryClawPluginDefinition } from "openclaw/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "openclaw/plugin-sdk/media-understanding";

const plugin: WineryClawPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
WineryClaw already treats image/audio/video understanding as one shared
...
`WineryClawPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   WineryClaw can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that WineryClaw knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `WineryClawPluginApi` or
  `api.runtime`
...
Native WineryClaw plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the WineryClaw process

Compatible bundles are safer by default because WineryClaw currently treats them
...
WineryClaw exports capabilities, not implementation convenience.
...
At startup, WineryClaw does roughly this:
...
The manifest is the control-plane source of truth. WineryClaw uses it to:
...
WineryClaw keeps short in-process caches for:
...
- Set `WINERYCLAW_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `WINERYCLAW_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `WINERYCLAW_PLUGIN_DISCOVERY_CACHE_MS` and
  `WINERYCLAW_PLUGIN_MANIFEST_CACHE_MS`.
...
WineryClaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, WineryClaw calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | WineryClaw tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                       |
...
that still runs on WineryClaw's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before WineryClaw's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as WineryClaw adds those
...
- WineryClaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "WineryClaw plugin runtime helpers",
    count: 5,
  },
});
```
...
- `openclaw/plugin-sdk/config-schema` for the root `wineryclaw.json` Zod schema
  export (`WineryClawSchema`).
...
- Bundled extension internals remain private. External plugins should use only
  `openclaw/plugin-sdk/*` subpaths. WineryClaw core/test code may use the repo
...
- Facade-loaded public entry points prefer the active runtime config snapshot
  ...
  WineryClaw is not yet serving a runtime snapshot.
...
`catalog.run(...)` returns the same shape WineryClaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to WineryClaw's
...
- if both `catalog` and `discovery` are registered, WineryClaw uses `catalog`
...
When WineryClaw needs setup surfaces for a disabled channel plugin, or
...
this flag. Keep the plugin on the default behavior and let WineryClaw load the
...
WineryClaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.wineryclaw/mpm/plugins.json`
- `~/.wineryclaw/mpm/catalog.json`
- `~/.wineryclaw/plugins/catalog.json`

Or point `WINERYCLAW_PLUGIN_CATALOG_PATHS` (or `WINERYCLAW_MPM_CATALOG_PATHS`) at
...
2. add typed plugin registration/runtime surfaces
   Extend `WineryClawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how WineryClaw stays opinionated without becoming hardcoded to one
