...
summary: "Create your first WineryClaw plugin in minutes"
read_when:
  - You want to create a new WineryClaw plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to WineryClaw
---

# Building Plugins

Plugins extend WineryClaw with new capabilities: channels, model providers,
...
You do not need to add your plugin to the WineryClaw repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`openclaw plugins install <package-name>`. WineryClaw tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect WineryClaw to a messaging platform (Discord, IRC, etc.)
  </Card>
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json openclaw.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to WineryClaw",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    WineryClaw also checks ClawHub before npm for bare package specs like
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, WineryClaw retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.
...
1. Watch for GitHub release tags on [openclaw/openclaw](https://github.com/openclaw/openclaw/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official WineryClaw X account [@openclaw](https://x.com/openclaw) for release announcements.
