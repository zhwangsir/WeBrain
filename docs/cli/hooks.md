...
Details:
  ...
  Homepage: docs/automation/hooks#session-memory
...
Enable a specific hook by adding it to your config (`~/.wineryclaw/wineryclaw.json` by default).
...
- Copies the hook pack into `~/.wineryclaw/hooks/<id>`
...
**Output:** `~/.wineryclaw/workspace/memory/YYYY-MM-DD-slug.md`
...
**Output:** `~/.wineryclaw/logs/commands.log`

**View logs:**

```bash
# Recent commands
tail -n 20 ~/.wineryclaw/logs/commands.log

# Pretty-print
cat ~/.wineryclaw/logs/commands.log | jq .

# Filter by action
grep '"action":"new"' ~/.wineryclaw/logs/commands.log | jq .
```
