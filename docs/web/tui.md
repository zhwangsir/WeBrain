...
- Local shell commands receive `WINERYCLAW_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `WINERYCLAW_THEME=light` before launching `openclaw tui`.
- To force the original dark palette instead, set `WINERYCLAW_THEME=dark`.
