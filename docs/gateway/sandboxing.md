---
summary: "How WineryClaw sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
WineryClaw can run **tools inside sandbox backends** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - noVNC observer access is password-protected by default; WineryClaw emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
Use `backend: "ssh"` when you want WineryClaw to sandbox `exec`, file tools, and media reads on
...
- WineryClaw creates a per-scope remote root under `sandbox.ssh.workspaceRoot`.
- On first use after create or recreate, WineryClaw seeds that remote workspace from the local workspace once.
...
- WineryClaw does not sync remote changes back to the local workspace automatically.

Authentication material:

- `identityFile`, `certificateFile`, `knownHostsFile`: use existing local files and pass them through OpenSSH config.
- `identityData`, `certificateData`, `knownHostsData`: use inline strings or SecretRefs. WineryClaw resolves them through the normal secrets runtime snapshot, writes them to temp files with `0600`, and deletes them when the SSH session ends.
...
- Host-local edits made outside WineryClaw after the seed step are not visible remotely until you recreate the sandbox.
...
Use `backend: "openshell"` when you want WineryClaw to sandbox tools in an
...
- `mirror` (default): local workspace stays canonical. WineryClaw syncs local files into OpenShell before exec and syncs the remote workspace back after exec.
- `remote`: OpenShell workspace is canonical after the sandbox is created. WineryClaw seeds the remote workspace once from the local workspace, then file tools and exec run directly against the remote sandbox without syncing changes back.

Remote transport details:

- WineryClaw asks OpenShell for sandbox-specific SSH config via `openshell sandbox ssh-config <name>`.
...
- Before `exec`, WineryClaw syncs the local workspace into the OpenShell sandbox.
- After `exec`, WineryClaw syncs the remote workspace back to the local workspace.
- File tools still operate through the sandbox bridge, but the local workspace remains the source of truth between turns.

Use this when:

- you edit files locally outside WineryClaw and want those changes to show up in the sandbox automatically
...
- When the sandbox is first created, WineryClaw seeds the remote workspace from the local workspace once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate directly against the remote OpenShell workspace.
- WineryClaw does **not** sync remote changes back into the local workspace after exec.
...
- If you edit files on the host outside WineryClaw after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `openclaw sandbox recreate` deletes the current runtime and lets WineryClaw recreate it on next use
...
- `"none"` (default): tools see a sandbox workspace under `~/.wineryclaw/sandboxes`.
...
WineryClaw mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- WineryClaw blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
- WineryClaw also blocks common home-directory credential roots such as `~/.aws`, `~/.cargo`, `~/.config`, `~/.docker`, `~/.gnupg`, `~/.netrc`, `~/.npm`, and `~/.ssh`.
- Bind validation is not just string matching. WineryClaw normalizes the source path, then resolves it again through the deepest existing ancestor before re-checking blocked paths and allowed roots.
...
- The three graphics hardening flags (`--disable-3d-apis`,
  ...
  when containers lack GPU support. Set `WINERYCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0`
...
- `--disable-extensions` is enabled by default and can be disabled with
  `WINERYCLAW_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
- `--renderer-process-limit=2` is controlled by
  `WINERYCLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
Set `WINERYCLAW_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can
override socket location with `WINERYCLAW_DOCKER_SOCKET`. Full setup and env
