...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “WineryClaw runs”).
...
Note: replace `18789` with your configured `gateway.port` (or `--port`/`WINERYCLAW_GATEWAY_PORT`).
...
- URL override safety:
  ...
  - Env URL overrides (`WINERYCLAW_GATEWAY_URL`) may use env credentials only (`WINERYCLAW_GATEWAY_TOKEN` / `WINERYCLAW_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `WINERYCLAW_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `WINERYCLAW_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `WINERYCLAW_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `WINERYCLAW_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `WINERYCLAW_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `WINERYCLAW_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
