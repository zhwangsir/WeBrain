...
summary: "Gateway config reference for core WineryClaw keys, defaults, and links to dedicated subsystem references"
read_when:
...
Core config reference for `~/.wineryclaw/wineryclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

This page covers the main WineryClaw config surfaces and links out when a subsystem has its own deeper reference. It does **not** try to inline every channel/plugin-owned command catalog or every deep memory/QMD knob on one page.
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — WineryClaw uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.wineryclaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- WineryClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
- `commands.callbackUrl` must resolve to the WineryClaw gateway endpoint and be reachable from the Mattermost server.
- Native slash callbacks are authenticated with the per-command tokens returned
  ...
  commands are activated, WineryClaw rejects callbacks with
...
WineryClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- `config: true` enables `/config` (reads/writes `wineryclaw.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
- `mcp: true` enables `/mcp` for WineryClaw-managed MCP server config under `mcp.servers`.
...
Optional repository root shown in the system prompt's Runtime line. If unset, WineryClaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `openai/gpt-5.4`). If you omit the provider, WineryClaw tries an alias first, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider (deprecated compatibility behavior, so prefer explicit `provider/model`). If that provider no longer exposes the configured default model, WineryClaw falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
...
- Environment overrides: `WINERYCLAW_AGENT_RUNTIME=<id|auto|pi>` overrides `runtime`; `WINERYCLAW_AGENT_HARNESS_FALLBACK=none` disables PI fallback for that process.
...
Replace the entire WineryClaw-assembled system prompt with a fixed string. Set at the default level (`agents.defaults.systemPromptOverride`) or per agent (`agents.list[].systemPromptOverride`). Per-agent values take precedence; an empty or whitespace-only value is ignored. Useful for controlled prompt experiments.
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before WineryClaw aborts it. Default: `900`.
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that WineryClaw materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.wineryclaw/sandboxes`
...
In `remote` mode, host-local edits made outside WineryClaw are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `wineryclaw.json`.
noVNC observer access uses VNC auth by default and WineryClaw emits a short-lived token URL (instead of exposing the password in the shared URL).
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from WINERYCLAW_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `WINERYCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `WINERYCLAW_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `WINERYCLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.wineryclaw/agents/main/agent",
...
For `type: "acp"` entries, WineryClaw resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, WineryClaw starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, WineryClaw treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
- Files are materialized into the child workspace at `.wineryclaw/attachments/<uuid>/` with a `.manifest.json`.
...
WineryClaw uses the built-in model catalog. Add custom providers via `models.providers` in config or `~/.wineryclaw/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `WINERYCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), WineryClaw forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
- `models.providers.*.models.*.compat.requiresStringContent`: optional compatibility hint for string-only OpenAI-compatible chat endpoints. When `true`, WineryClaw flattens pure text `messages[].content` arrays into plain strings before sending the request.
...
`openai-completions` transport, and WineryClaw keys that off endpoint capabilities
...
On the Anthropic-compatible streaming path, WineryClaw disables MiniMax thinking
...
- Loaded from `~/.wineryclaw/extensions`, `<workspace>/.wineryclaw/extensions`, plus `plugins.load.paths`.
- Discovery accepts native WineryClaw plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native WineryClaw plugin schema when available).
...
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; WineryClaw applies those as sanitized agent settings, not as raw WineryClaw config patches.
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want WineryClaw to discover `/json/version`; use WS(S)
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or WINERYCLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `WINERYCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `WINERYCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `WINERYCLAW_APNS_RELAY_BASE_URL` / `WINERYCLAW_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `WINERYCLAW_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
WINERYCLAW_CONFIG_PATH=~/.wineryclaw/a.json \
WINERYCLAW_STATE_DIR=~/.openclaw-a \
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `WINERYCLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.wineryclaw/workspace/canvas",
    liveReload: true,
    // enabled: false, // or WINERYCLAW_SKIP_CANVAS_HOST=1
  },
}
```
...
- Hostname defaults to `openclaw`. Override with `WINERYCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.wineryclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.
...
- `.env` files: CWD `.env` + `~/.wineryclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${WINERYCLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `wineryclaw.json` credential paths.
...
- Legacy OAuth imports from `~/.wineryclaw/credentials/oauth.json`.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.wineryclaw/logs/cache-trace.jsonl",
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$WINERYCLAW_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one WineryClaw.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `WINERYCLAW_HIDE_BANNER=1`.
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `wineryclaw.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
