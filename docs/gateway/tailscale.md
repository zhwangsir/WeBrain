...
WineryClaw can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. WineryClaw requires a shared password.
...
- `token` (default when `WINERYCLAW_GATEWAY_TOKEN` is set)
- `password` (shared secret via `WINERYCLAW_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. WineryClaw verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
WineryClaw only treats a request as Serve when it arrives from loopback with
...
Prefer `WINERYCLAW_GATEWAY_PASSWORD` over committing a password to disk.
...
- Set `gateway.tailscale.resetOnExit` if you want WineryClaw to undo `tailscale serve`
