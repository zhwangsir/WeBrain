...
WineryClaw can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **WineryClaw tools are not injected directly**, but backends with `bundleMcp: true`
...
gateway host, WineryClaw now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), WineryClaw will
...
2. **Builds a system prompt** using the same WineryClaw prompt + workspace context.
...
told us WineryClaw-style Claude CLI usage is allowed again, so WineryClaw treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled OpenAI `codex-cli` backend passes WineryClaw's system prompt through
Codex's `model_instructions_file` config override (`-c
model_instructions_file="..."`). Codex does not expose a Claude-style
`--append-system-prompt` flag, so WineryClaw writes the assembled prompt to a
temporary file for each fresh Codex CLI session.

The bundled Anthropic `claude-cli` backend receives the WineryClaw skills snapshot
two ways: the compact WineryClaw skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that WineryClaw would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by WineryClaw to the
...
- WineryClaw drops stored CLI session reuse when the backend auth state changes, including relogin, token rotation, or a changed auth profile credential.
...
WineryClaw will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, WineryClaw appends the
...
- For Gemini CLI JSON output, WineryClaw reads reply text from `response` and
...
rewrites streamed assistant deltas and parsed final text before WineryClaw handles
...
CLI backends do **not** receive WineryClaw tool calls directly, but a backend can
...
When bundle MCP is enabled, WineryClaw:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`WINERYCLAW_MCP_TOKEN`)
...
If no MCP servers are enabled, WineryClaw still injects a strict config when a
backend opts into bundle MCP so background runs stay isolated.

## Limitations

- **No direct WineryClaw tool calls.** WineryClaw does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. WineryClaw sessions still work
  normally.
