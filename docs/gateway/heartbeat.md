...
- During heartbeat runs, WineryClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- If only alert delivery is disabled, WineryClaw can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
...
If **all three** are false, WineryClaw skips the heartbeat run entirely (no model call).
...
- WineryClaw parses the `tasks:` block and checks each task against its own `interval`.
