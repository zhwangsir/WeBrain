...
// Prefix for WineryClaw-generated action IDs to scope our handler
const WINERYCLAW_ACTION_PREFIX = "openclaw:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${WINERYCLAW_ACTION_PREFIX}`);

  // Handle WineryClaw modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
