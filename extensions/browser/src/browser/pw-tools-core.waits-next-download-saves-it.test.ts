...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredWineryClawTmpDir: vi.fn(() => "/tmp/openclaw"),
}));
...
describe("pw-tools-core", () => {
  beforeAll(async () => {
    ...
    vi.spyOn(tmpDirModule, "resolvePreferredWineryClawTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredWineryClawTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredWineryClawTmpDir.mockReturnValue("/tmp/openclaw");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredWineryClawTmpDir.mockReturnValue("/tmp/openclaw-preferred");
    ...
    expect(tmpDirMocks.resolvePreferredWineryClawTmpDir).toHaveBeenCalled();
  });
