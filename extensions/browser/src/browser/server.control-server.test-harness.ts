...
vi.mock("./chrome.js", () => ({
  ...
  launchWineryClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.WINERYCLAW_GATEWAY_PORT;
  process.env.WINERYCLAW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.WINERYCLAW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.WINERYCLAW_GATEWAY_PASSWORD;
