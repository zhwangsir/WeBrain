...
function resolveMemoryPluginConfig(cfg: WineryClawConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredWineryClawTmpDir(), "openclaw-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: WineryClawConfig): string {
...
function resolveAgent(cfg: WineryClawConfig, agent?: string) {
...
function resolveAgentIds(cfg: WineryClawConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredWineryClawTmpDir(), "openclaw-rem-backfill-"),
      );
      try {
