...
export function resolveCodexAppServerRuntimeOptions(
  ...
  const command =
    readNonEmptyString(config.command) ?? env.WINERYCLAW_CODEX_APP_SERVER_BIN ?? "codex";
  const args = resolveArgs(config.args, env.WINERYCLAW_CODEX_APP_SERVER_ARGS);
  ...
  return {
    ...
    approvalPolicy:
      ...
      resolveApprovalPolicy(env.WINERYCLAW_CODEX_APP_SERVER_APPROVAL_POLICY) ??
      "never",
    sandbox:
      resolveSandbox(config.sandbox) ??
      resolveSandbox(env.WINERYCLAW_CODEX_APP_SERVER_SANDBOX) ??
      "workspace-write",
    approvalsReviewer:
      resolveApprovalsReviewer(config.approvalsReviewer) ??
      (env.WINERYCLAW_CODEX_APP_SERVER_GUARDIAN === "1" ? "guardian_subagent" : "user"),
