...
`openclaw.invoke --each`) without adding new WineryClaw code per workflow.
...
This extension depends on WineryClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** WineryClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.wineryclaw/extensions` as a standalone plugin directory.
