...
import type { WineryClawPluginApi, WineryClawPluginToolContext } from "../api.js";
...
describe("diffs plugin registration", () => {
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<WineryClawPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: WineryClawPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<WineryClawPluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as WineryClawPluginApi);
