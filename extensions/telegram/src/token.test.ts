...
describe("resolveTelegramToken", () => {
  ...
  function createUnknownAccountConfig(): WineryClawConfig {
    return {
      channels: {
        telegram: {
  ...
  function expectNoTokenForUnknownAccount(cfg: WineryClawConfig) {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked tokenFile paths", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile: tokenLink } } } as WineryClawConfig;
