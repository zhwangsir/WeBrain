...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredWineryClawTmpDir: vi.fn(() => path.join(os.tmpdir(), "openclaw-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredWineryClawTmpDir: pluginApiMocks.resolvePreferredWineryClawTmpDir,
...
function createApi(params?: {
  runtime?: WineryClawPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as WineryClawPluginApi["runtime"],
...
function createChannelRuntime(
  ...
  } as unknown as WineryClawPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredWineryClawTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredWineryClawTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the WineryClaw iOS app:");
    expect(text).toContain("![WineryClaw pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the WineryClaw iOS app:");
