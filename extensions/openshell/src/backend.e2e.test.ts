...
const WINERYCLAW_OPENSHELL_E2E = process.env.WINERYCLAW_E2E_OPENSHELL === "1";
const WINERYCLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const WINERYCLAW_OPENSHELL_COMMAND =
  process.env.WINERYCLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && WINERYCLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: WINERYCLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(WINERYCLAW_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(WINERYCLAW_OPENSHELL_COMMAND))) {
        return;
      }
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: WINERYCLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
