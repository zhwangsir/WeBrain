    ...
    if (!stat.isDirectory()) {
      return {
        ...
        warning:
          ...
          "WineryClaw skipped automatic crypto migration for that path.",
      };
    }
  } catch (err) {
    return {
...
function resolveMatrixAccountIds(cfg: WineryClawConfig): string[] {
    ...
    if (!plan.deviceId) {
      warnings.push(
        `Legacy Matrix encrypted state detected at ${plan.legacyCryptoPath}, but no device ID was found for account "${plan.accountId}". ` +
          `WineryClaw will continue, but old encrypted history cannot be recovered automatically.`,
      );
      continue;
    }
