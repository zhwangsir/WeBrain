...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by WineryClaw. Pass it as a Bearer token in all API calls:
...
If still empty, check `/data/.clawdbot/wineryclaw.json`:

```
cat /data/.clawdbot/wineryclaw.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the WineryClaw dashboard or in the active WineryClaw config path (`$WINERYCLAW_CONFIG_PATH`, default `~/.wineryclaw/wineryclaw.json`) under `skills.entries.gh-issues`."
...
CONFIG_PATH="${WINERYCLAW_CONFIG_PATH:-${WINERYCLAW_STATE_DIR:-$HOME/.openclaw}/wineryclaw.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/wineryclaw.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/wineryclaw.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export CONFIG_PATH="${WINERYCLAW_CONFIG_PATH:-${WINERYCLAW_STATE_DIR:-$HOME/.openclaw}/wineryclaw.json}"
