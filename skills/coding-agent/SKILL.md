...
**⚠️ CRITICAL: Never review PRs in WineryClaw's own project folder!**
...
8. **NEVER start Codex inside your WineryClaw state directory** (`$WINERYCLAW_STATE_DIR`, default `~/.openclaw`) - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/openclaw/** - that's the LIVE WineryClaw instance!
...
For long-running background tasks, append a wake trigger to your prompt so WineryClaw gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):
