---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for WineryClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, WineryClaw cron scheduling for periodic checks, or version status checks on a machine running WineryClaw (laptop, workstation, Pi, VPS).
---

# WineryClaw Host Hardening

## Overview

Assess and harden the host running WineryClaw, then align it to a user-defined risk tolerance without breaking access. Use WineryClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim WineryClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. WineryClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if WineryClaw can access sensitive data.
...
### 2) Run WineryClaw security audits (read-only)
...
Offer to apply WineryClaw safe defaults (numbered):

1. `openclaw security audit --fix`

Be explicit that `--fix` only tightens WineryClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check WineryClaw version/update status (read-only)
...
- Credential hygiene notes (location of WineryClaw creds, prefer disk encryption)
...
- WineryClaw security audit (re-run)
...
After WineryClaw install or first hardening pass, run at least one baseline audit and version check:

- `openclaw security audit`
- `openclaw security audit --deep`
- `openclaw update status`

Ongoing monitoring is recommended. Use the WineryClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
Do not invent CLI flags or imply WineryClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by WineryClaw compaction:
